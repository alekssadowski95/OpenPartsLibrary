...
from openpartslibrary.forms import CreateComponentForm, CreateSupplierForm, CreateMaterialForm, LoginForm, RegistrationForm, CreateFileForm
...
sample_spreadsheet_path = pl.get_default_sample_spreadsheet_path()
if not pl.session.query(Component).first():
    pl.import_from_spreadsheet(
        sample_spreadsheet_path,
        components_cad_dir_path=pl.sample_data_dir_path / "components-cad",
    )
pl.sync_cad_files_from_spreadsheet(sample_spreadsheet_path)
...
def components(search_query):
    ...
    return render_template('component/component-list.html', components = components, len = len, search_query = search_query, user = current_user, component_cad_filenames = component_cad_filenames)
...
def component_view(uuid):
    ...
    used_in_files = []
    return render_template('component/component-read.html', component = component, len = len, component_cad_filepath = component_cad_filepath, component_cad_filename = component_cad_filename, files = files, used_in_files = used_in_files) 
